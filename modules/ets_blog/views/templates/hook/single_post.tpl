...
<div class="ets_blog_layout_{$blog_layout|escape:'html':'UTF-8'} ets-blog-wrapper-detail" itemscope itemType="http://schema.org/newsarticle">
    ...
    {if $blog_post}
                ...
                <div class="ets-blog-latest-toolbar">
                    {if $allow_rating && $everage_rating}                      
                        <div class="blog_rating_wrapper">                            
                            {if $total_review}
                                <span title="{l s='Komentarai' mod='ets_blog'}" class="blog_rating_reviews">
                                     <span class="total_views">{$total_review|intval}</span>
                                     <span>
                                        {if $total_review != 1}
                                            {l s='Komentarai' mod='ets_blog'}
                                        {else}
                                            {l s='Komentaras' mod='ets_blog'}
                                        {/if}
                                    </span>
                                </span>
                            {/if}
                            <div title="{l s='Vidutinis įvertinimas' mod='ets_blog'}" class="ets_blog_review">
                    ...
                    {if $show_date}
                        ...
                        <span class="post-date">
                            <span class="be-label">{l s='Paskelbta' mod='ets_blog'}: </span>
                    ...
                    {if $show_author && isset($blog_post.employee) &&  $blog_post.employee}
                        <div class="author-block" itemprop="author" itemscope itemtype="http://schema.org/Person">
                            <span class="post-author-label">{l s=' ' mod='ets_blog'}</span>
            ...
            <div class="extra_tag_cat">
                {if $show_tags && $blog_post.tags}
                    <div class="ets-blog-tags">
                        ...
                        <span class="be-label">
                            {if $totalTag > 1}{l s='Žymos' mod='ets_blog'}
                    ...
                    <div class="ets-des-and-author">
                        <div class="ets-author-name">
                            <a href="{$blog_post.author_link|escape:'html':'UTF-8'}">
                                {l s='Autorius' mod='ets_blog'}: {$blog_post.employee.name|escape:'html':'UTF-8'}
                            </a>
                        </div>
            ...
            <div class="ets-blog-wrapper-comment"> 
                {if $allowComments}
                    <div class="ets_comment_form_blog">
                        <h4 class="title_blog">{l s='Palikti komentarą' mod='ets_blog'}</h4>
                        <div class="ets-blog-form-comment" id="ets-blog-form-comment">                   
                                ...
                                <form action="#ets-blog-form-comment" method="post">
                                    ...
                                    {if !$hasLoggedIn} 
                                        <div class="blog-comment-row blog-name">
                                            <label for="bc-name">{l s='Vardas' mod='ets_blog'}</label>
                                        ...
                                        <div class="blog-comment-row blog-email">
                                            <label for="bc-email">{l s='El. Paštas' mod='ets_blog'}</label>
                                    ...
                                    <div class="blog-comment-row blog-title">
                                        <label for="bc-subject">{l s='Tema ' mod='ets_blog'}</label>
                                    ...
                                    <div class="blog-comment-row blog-content-comment">
                                        <label for="bc-comment">{l s='Komentaras ' mod='ets_blog'}</label>
                                            ...
                                            <div class="blog-rate-capcha">
                                                {if $allow_rating}                            
                                                    <div class="blog-rate-post">
                                                        <label>{l s='Įvertinimas: ' mod='ets_blog'}</label>
                                        ...
                                        <div class="blog-submit-form">                                                    
                                            {if !Configuration::get('ETS_BLOG_DISPLAY_GDPR_NOTIFICATION')}
                                                <div class="blog-submit">
                                                    <input class="button" type="submit" value="{l s='Pridėti komentarą' mod='ets_blog'}" name="bcsubmit" />
                                                </div>
                                            {/if}   
                                        </div>            
                                    ...
                                    <div class="blog-comment-row">                                            
                                        <div class="blog-submit-form">
                                            {if Configuration::get('ETS_BLOG_DISPLAY_GDPR_NOTIFICATION')}
                                                ...
                                                <div class="blog-submit">
                                                    <input class="button" type="submit" disabled="disabled" value="{l s='Pridėti komentarą' mod='ets_blog'}" name="bcsubmit" />
                                                </div>
                                            {/if}   
                                        </div>  
                                    </div>                                            
                            ...
                            {else}
                                <p class="alert alert-warning">{l s='Prisijunkite kad galėtumėte komentuoti' mod='ets_blog'}</p>
                            {/if}
                        </div> 
                    </div>
                    {if count($comments)}
                        <div class="ets_blog-comments-list">
                        <h4 class="title_blog">{l s='Komentarai' mod='ets_blog'}</h4>
                            <ul id="blog-comments-list" class="blog-comments-list">
                                {foreach from=$comments item='comment'}
                                        ...
                                        <div class="ets-blog-detail-comment">
                                            ...
                                            {if $comment.name}<span class="comment-by">{l s=' ' mod='ets_blog'}<b>{ucfirst($comment.name)|escape:'html':'UTF-8'}</b></span>{/if}
                                            ...
                                            {if $allow_rating && $comment.rating > 0}
                                                <div class="comment-rating" >
                                                    <span>{l s='Įvertinimas: ' mod='ets_blog'}</span>
        ...
        {else}
            <p class="warning">{l s='Įrašų nerasta' mod='ets_blog'}</p>
        {/if}
        {if $blog_post.related_posts}
            <div class="ets-blog-related-posts ets_blog_related_posts_type_{if $blog_related_posts_type}{$blog_related_posts_type|escape:'html':'UTF-8'}{else}default{/if}">
                <h4 class="title_blog">{l s='Susiję įrašai' mod='ets_blog'}</h4>
                                            ...
                                            {if $rpost.comments_num > 0 }                                                                
                                                <span class="ets-blog-latest-toolbar-comments">{$rpost.comments_num|intval}
                                                    {if $rpost.comments_num!=1}
                                                        <span>{l s='komentarai' mod='ets_blog'}</span>
                                                    {else}
                                                        <span>{l s='komentaras' mod='ets_blog'}</span>
                                                    {/if}
                                                </span> 
                                            {/if}                                                                    
                                        {/if}
