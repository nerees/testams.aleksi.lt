...
<div class="ets_blog_layout_{$blog_layout|escape:'html':'UTF-8'} ets-blog-wrapper ets-blog-wrapper-blog-list{if isset($blog_config.ETS_BLOG_AUTO_LOAD) &&$blog_config.ETS_BLOG_AUTO_LOAD} loadmore{/if}">
    {if $blog_categories}
        <h2 class="page-heading product-listing">{l s='Visos kategorijos' mod='ets_blog'}</h2>
                                ...
                                {if ( $category.count_posts > 0 )}
                                    {if ( $category.count_posts == 1 )}
                                        <a class="view_detail_link blog_view_all" href="{$category.link|escape:'html':'UTF-8'}">
                                            {l s='Skaityti %d įrašą' sprintf=[$category.count_posts] mod='ets_blog'}
                                        </a>  
                                     {else}
                                        <a class="view_detail_link blog_view_all" href="{$category.link|escape:'html':'UTF-8'}">
                                            {l s='Skaityti %d įrašus' sprintf=[$category.count_posts] mod='ets_blog'}
                                        </a> 
                                     {/if}  
                                {else}
                                    <a class="view_detail_link blog_view_all" href="{$category.link|escape:'html':'UTF-8'}">
                                        {l s='Detaliau' mod='ets_blog'}
                                    </a>
                                {/if}
                             </div>
                        </div>
        ...
        {/if}
    {else}
        <p>{l s='Nerasta kategorija' mod='ets_blog'}</p>
    {/if}
    </div> 