   ...
   {        
    ...
    public function getLink($controller = 'blog', $params = array(),$id_lang=0)
    {
        ...
        {    
            ...
            {
                $url .= 'kategorijos'.($page ? '/'.rtrim($page,'/') : '');
                return $url;
            }
            elseif($controller=='comment')
            {
                $url .= 'komentarai'.($page ? '/'.rtrim($page,'/') : '');
                return $url;
            }
            elseif($controller=='blog')
            {
                ...
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_POST',$id_lang)) ? $subAlias : 'irasas').'/'.(int)$params['id_post'].'-'.(int)$params['edit_comment'].'-'.$postAlias.$subfix;
                }
                ...
                {
                    $url .= 'irasas/komentarai/'.(int)$params['id_post'].'-'.$postAlias.$subfix;
                }
                elseif(isset($params['id_post']) && $postAlias = $this->getPostAlias((int)$params['id_post'],$id_lang))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_POST',$id_lang)) ? $subAlias : 'irasas').'/'.$params['id_post'].'-'.$postAlias.$subfix;
                }
                elseif(isset($params['id_category']) && $categoryAlias = $this->getCategoryAlias((int)$params['id_category'],$id_lang))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_CATEGORY',$id_lang)) ? $subAlias : 'kategorija').($page ? '/'.rtrim($page) : '/').$params['id_category'].'-'.$categoryAlias.$subfix;
                }
                elseif(isset($params['id_author']) && isset($params['is_customer']) && $params['is_customer'] && $authorAlias = (isset($params['alias']) ? $params['alias'] : 'autorius'))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_AUTHOR2',$id_lang)) ? $subAlias : 'autorius').($page ? '/'.rtrim($page) : '/').(int)$params['id_author'].'-'.$authorAlias;
                }
                elseif(isset($params['id_author']) && $authorAlias = (isset($params['alias']) ? $params['alias'] : 'autorius'))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_AUTHOR',$id_lang)) ? $subAlias : 'autorius').'/'.$page.(int)$params['id_author'].'-'.$authorAlias;
                }
                elseif(isset($params['tag']))
                {
                    $url .= $page.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_TAG',$id_lang)) ? $subAlias : 'zyme').'/'.(string)$params['tag'];
                }
                elseif(isset($params['search']))
                {
                    $url .= $page.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_SEARCH',$id_lang)) ? $subAlias : 'paieska').'/'.(string)$params['search'];
                }
                elseif(isset($params['latest']))
                {
                    $url .= 'naujausi'.($page ? '/'.rtrim($page,'/') : '');
                }
                elseif(isset($params['popular']))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_POPULAR',$id_lang)) ? $subAlias : 'populiarus').($page ? '/'.rtrim($page,'/') : '');
                }
                elseif(isset($params['featured']))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_FEATURED',$id_lang)) ? $subAlias : 'prisegtas').($page ? '/'.rtrim($page,'/') : '');
                }
                elseif(isset($params['month']) && isset($params['year']))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_MONTHS',$id_lang)) ? $subAlias : 'menuo').'/'.$params['month'].'/'.$params['year'].($page ? '/'.rtrim($page,'/') : '');
                }
                elseif(isset($params['year']))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_YEARS',$id_lang)) ? $subAlias : 'metai').'/'.$params['year'].($page ? '/'.rtrim($page,'/') : '');
                }
                else
                {
                    if($page)
    ...
    public function hookModuleRoutes()
    {
        ...
        $routes = array(
            ...
            'etsblogpost2' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/irasas/{id_post}-{url_alias}'.$subfix,
                'keywords' => array(
            ...
            'etsblogpost' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/irasas/{post_url_alias}'.$subfix,
                'keywords' => array(
            ...
            'etsblogcategorypostpage2' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/kategorija/{page}/{id_category}-{url_alias}'.$subfix,
                'keywords' => array(
            ...
            'etsblogcategorypostpage' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/kategorija/{page}/{category_url_alias}'.$subfix,
                'keywords' => array(
                    'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
            ...
            'etsblogcategorypost2' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/kategorija/{id_category}-{url_alias}'.$subfix,
                'keywords' => array(
                    'id_category' =>    array('regexp' => '[0-9]+', 'param' => 'id_category'),
                    'url_alias'       =>   array('regexp' => '[_a-zA-Z0-9-\pL]+','param' => 'url_alias'),
                ),
            ...
            'etsblogcategorypost' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/kategorija/{category_url_alias}'.$subfix,
                'keywords' => array(
                    'category_url_alias'       =>   array('regexp' => '[_a-zA-Z0-9-\pL]+','param' => 'category_url_alias'),
                ),
                'params' => array(
            ...
            'etsblogcategoriespage' => array(
                'controller' => 'category',
                'rule' => $blogAlias.'/kategorijos/{page}',
                'keywords' => array(
                    'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                ),
                'params' => array(
            ...
            'etsblogauthorpost' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/autorius/{id_author}-{author_name}',
                'keywords' => array(
            ...
            'etsblogauthorpostpage' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/autorius/{page}/{id_author}-{author_name}',
                'keywords' => array(
                    'id_author' =>    array('regexp' => '[0-9]+', 'param' => 'id_author'),
                    'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                    'author_name'       =>   array('regexp' => '(.)+','param' => 'author_name'),
            ...
            'etsblogpostinyearpage' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/metai/{year}/{page}',
                'keywords' => array(
            ...
            'etsblogpostinmonthpage' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/menuo/{month}/{year}/{page}',
                'keywords' => array(
            ...
            'etsblogpostinmonth' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/menuo/{month}/{year}',
                'keywords' => array(
                    'month'       =>   array('regexp' => '[0-9]+','param' => 'month'),
                    'year'       =>   array('regexp' => '[0-9]+','param' => 'year'),
                ),
            ...
            'etsblogpostallcomments' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/irasas/komentarai/{id_post}-{url_alias}'.$subfix,
                'keywords' => array(
                    'id_post' =>    array('regexp' => '[0-9]+', 'param' => 'id_post'),
                    'url_alias'       =>   array('regexp' => '[_a-zA-Z0-9-\pL]+','param' => 'url_alias'),
                ),
            ...
            'etsblogsearchpage' => array(
                'controller' => 'blog',
                'rule' => $blogAlias.'/{page}/paieska/{search}',
                'keywords' => array(
    ...
    public function getMonthName($month)
    {
        switch ($month) {
            case 1:
                return $this->l('Sausis');
            case 2:
                return $this->l('Vasaris');
            case 3:
                return $this->l('Kovas');
            case 4:
                return $this->l('Balandis');
            case 5:
                return $this->l('Gegužė');
            case 6:
                return $this->l('Birželis');
            case 7:
                return $this->l('Liepa');
            case 8:
                return $this->l('Rugpjūtis');
            case 9:
                return $this->l('Rugsėjis');
            case 10:
                return $this->l('Spalis');
            case 11:
                return $this->l('Lapkritis');
            case 12:
                return $this->l('Gruodis');
        }
    }
    ...
    public function setMetas()
    {
        ...
        if($id_category)
        {
            if(($category = new Ets_blog_category($id_category,$id_lang)) && Validate::isLoadedObject($category))
            {
                ...
                $meta['meta_title'] = $this->l('Puslapis nerastas');
        ...
        {
            $meta['meta_title'] = $this->l('Visos kategorijos');
        }
        elseif($controller=='blog')
        {
    ...
    public function installDefaultCategory()
    {
        ...
        foreach($languages as $lang)
        {
            $category->title[$lang['id_lang']] = $lang['id_lang']= $this->context->language->id ? $this->l('Bendra kategorija') : $this->getTextLang('Bendra kategorija',$lang['id_lang']);
        }
        return $category->add();
