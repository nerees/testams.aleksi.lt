...
{if $posts}
    <div class="block ets_block_latest {$ETS_BLOG_RTL_CLASS|escape:'html':'UTF-8'} {if isset($page) && $page}page_{$page|escape:'html':'UTF-8'}{else}page_blog{/if} ets_block_slider">
        <h4 class="title_blog title_block">{l s='Naujausi įrašai' mod='ets_blog'}</h4>
        ...
        <div class="block_content row">
                        ...
                        <div class="ets-blog-latest-post-content">
                            ...
                            {if $allowComments || $show_views || $allow_like} 
                                <div class="ets-blog-latest-toolbar">
                                    {if $show_views}
                                        <span class="ets-blog-latest-toolbar-views">{$post.click_number|intval} {if $post.click_number!=1}<span>{l s='peržiūros' mod='ets_blog'}</span>{else}<span>{l s='peržiūra' mod='ets_blog'}</span>{/if}</span>
                                    {/if}   
                                    {if $allowComments && $post.comments_num>0}
                                        ...
                                        {l s='komentarai' mod='ets_blog'}</span>{else}<span>{l s='komentaras' mod='ets_blog'}</span>{/if}</span>
                                    {/if}                                 
                                    {if $allow_like}
                                        <span title="{if $post.liked}{l s='Patiko' mod='ets_blog'}{else}{l s='Pamėgti šį įrašą' mod='ets_blog'}{/if}" class="ets-blog-like-span ets-blog-like-span-{$post.id_post|intval} {if $post.liked}active{/if}"  data-id-post="{$post.id_post|intval}">
                                            <span class="ben_{$post.id_post|intval}">{$post.likes|intval}</span>
                                            <span class="blog-post-like-text blog-post-like-text-{$post.id_post|intval}">{l s='Pamėgtas' mod='ets_blog'}</span>
                            ...
                            {/if}
                            <a class="read_more" href="{$post.link|escape:'html':'UTF-8'}">{if $ets_blog_text_Readmore}{$ets_blog_text_Readmore|escape:'html':'UTF-8'}{else}{l s='Skaityti daugiau' mod='ets_blog'}{/if}</a>
                        </div>
                    </li>
                {/foreach}
            </ul>
            {if $page!='home'}
                <div class="blog_view_all_button">
                    <a href="{$latest_link|escape:'html':'UTF-8'}" class="view_all_link">{l s='Skaityti visus' mod='ets_blog'}</a>
                </div>
            {/if}
        </div>
        <div class="clear"></div>
